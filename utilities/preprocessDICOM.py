...
def preprocess(dicom_file, rtstruct_file, mask_background_value=0, mask_foreground_value=255, zero=False,
    ...
    # for i in rtstructs:
    #     if (i['name'] == 'gtv_1') or (i['name'] == 'gtv__1'):
    #         rtstruct_contours = i['sequence']
    #     else:
    #         print(f'Structure(gtv_1 or gtv__1) not found. Found {i["name"]} Structure')
    if str_names is not None:
        donestr = []
        for i in rtstructs:
            if i['name'] in str_names:
                print(f"Structure: {i['name']} found")
                rtstruct_contours = i['sequence']
                donestr.append(i['name'])
        if len(donestr) == 0:
            print(f'Structures {str_names} not found.')
            return None
    else:
        print("No reference structure for cropping")
        return None
    dcm_patient_coords_to_mask = DcmPatientCoords2Mask()
    ...
    for contour in rtstruct_contours:
        ...
        for index in range(0, len(coordinates['x'])):
            ...
            world_coords = dicom_image.TransformPhysicalPointToIndex(
                (coordinates['x'][index], coordinates['y'][index], coordinates['z'][index]))
